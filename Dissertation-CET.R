...
CET.A.Q <- CET[, 8:16]
CET.B.Q <- CET[, 17:25]
CET.A.Qtrue <- na.omit(CET.A.Q)
length(CET.A.Qtrue[,1])
CET.B.Qtrue <- na.omit(CET.B.Q)
length(CET.B.Qtrue[,1])
...
corre_exe.A
corre_exe.A.p
# item 9 is postive correlated with Stroop 


corre_exe.B <- matrix(0,9,3)
corre_exe.B.p <- matrix(0,9,3)
for (i in 1:9) {
  for (j in 1:3) {
